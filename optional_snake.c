...
void speed_read_state_machine(void) {
	...
	while(1)
	{
		...
		switch (state)
		{
			case 0:
				speed=1;
				if(XGpio_DiscreteRead(&sws, 1) & 8) state=1;

				break;
			case 1:
				speed=2;
				if(!(XGpio_DiscreteRead(&sws, 1) & 8)) state=0;
				break;
		}
	}
	vTaskDelete(NULL);
...
int main (void){
	...
	xTaskCreate ((void(*)(void*))sequence_state_machine, "task1", 200, NULL, 1, NULL);
	xTaskCreate ((void(*)(void*))control_state_machine, "task2", 200, NULL, 1, NULL);
	xTaskCreate ((void(*)(void*))control_menjar, "task3", 200, NULL, 1, NULL);
	xTaskCreate ((void(*)(void*))speed_read_state_machine, "task4", 200, NULL, 1, NULL);
